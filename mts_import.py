...
DELIMITER ',' CSV; ''' % (path,a,path,a,path,a)

cur_step1 = conPG.cursor()
cur_step1.execute(insert_table)
print("STEP2:insert data successfully")

##STEP_00  update RUTTING ####################################################################################################################################
rut_up = """
        update access_valuelaser a set rutt_right = rutt_right_p ,rutt_left = rutt_left_p,rutting=rutting_p
        from
            (select chainage+5 as chainage, (rutt_right+lag(rutt_right) over (order by chainage))/2 as rutt_right_p,
                            (rutt_left+lag(rutt_left) over (order by chainage))/2 as rutt_left_p,
                            (rutting+lag(rutting) over (order by chainage))/2 as rutting_p
                from
                    (SELECT right((cast(chainage as varchar(10))),1) as chainage_s,chainage,chainage-5 as chainage_o,
                     chainage+5 as chainage_d ,rutt_left, rutting,rutt_right
                    FROM public.access_valuelaser
                    order by chainage) foo
                where chainage_s ='0') b
        where a.chainage = b.chainage
"""
cur_step101 = conPG.cursor()
cur_step101.execute(rut_up)
print("STEP2.1:update RUTTING")
conPG.commit()
...
irmp = '''
select link_id,chainage,iri,texture,frame_number
		from data_suvey
		where iri is null or texture is null or frame_number is null
		and split_part((chainage/ CAST(25 AS float))::text, '.', 2) = ''  '''
...
create table survey as --delete from survey_local
	...
	date, the_geom, 'MS_survey'::character(10) as remark, 'MS22'::character(10) as run_new, 25::int as interval
	from
	(
...
select 'ms22/cu_survey/'||left(a.link_id,3)||'/'||date||'/'||survey_code||'/Run'||run_code||'/image/'||filename as directory,
filename, date, a.chainage as img_id,km, 'True'::text as imagepath, the_geom, survey_id, 'MS_22'::character(10) as remark 
from
(
...
##STEP_100  create mims_export.csv  ####################################################################################################################################
step100 =   '''create table mims_export as
select * 
from	
	(select road_id,road_revision, 'null' as section_id,section_revision,b.section_part_id,b.route as road_code,b.control as section_code,
	km_s as km_start, km_en as km_end,
	iri,rut,mpd,image_url,the_geom
	from		
		(select *,case when lane_group < 0 then km_e+chainage_iv2 
				 when lane_group > 0 then km_e-chainage_iv2 end km_s,km_e as km_en
		from
			(select *,
						case when lane_group < 0 then km_start-km_sum 
						when lane_group > 0 then km_start+km_sum end km_e			
			from
				(select chainage,chainage_iv2::int,section_id,route,control,km_start,km_end,iri,rut,mpd,image_url,lane_group,the_geom,
					SUM(chainage_iv2) OVER (PARTITION BY km_start ORDER BY chainage)::int as km_sum 
				from

					 (select *,case when chainage_iv is null then  25 
							when chainage_iv is not null then chainage_iv end chainage_iv2
					 from
						(select  chainage,(lead(chainage) over (order by chainage ROWS BETWEEN CURRENT ROW and 1 FOLLOWING )-chainage) as chainage_iv,		
								b.section_id::int,b.route::int ,b.control::int,b.km_start,b.km_end,
								iri,rut,mpd,image_url,lane_group,a.the_geom 
						from
							(select chainage,a.survey_id,survey_point_id,iri,rutting as rut,texture as mpd,directory as image_url ,a.the_geom 
							from survey_point_local a
							left join survey_image b on a.the_geom = b.the_geom) a
						left join 
							(SELECT survey_id,section_id,link_id,lane_group,substring(link_id,4,4) as route,substring(link_id,8,4) as control ,km_start,km_end 
							 from survey) b                                                                                                                                           
						on a.survey_id = b.survey_id)foo)foo
				order by chainage)foo)foo
		) a
left join
		(SELECT road_id,road_revision,section_revision,section_part_id,route::int,control::int
		FROM public.zz_mims_data) b
on a.route = b.route and a.control = b.control and a.section_id = b.section_part_id) foo'''
cur_step100 = conPG.cursor()
cur_step100.execute(step100)
conPG.commit()
print('step : create mims_export.csv  successfully')
...
##STEP_110  Export CSV  ####################################################################################################################################
dump_csv = pd.read_sql_query('''select * from mims_export''',conPG)
df = pd.DataFrame(dump_csv)
df.to_csv(r'%s\%s\%s.csv'%(path,a,a),index=True)
print('step : Export CSV  successfully')
...
dump4 = r'''COPY (
SELECT 'mkdir' as test,'E:\'||'s22\image\'||left(replace(directory,'/','\'),-26) as folder
FROM survey_image
group by test,folder
) TO '%s\%s\mkdir.bat'; ''' % (path,a)
cur_dump4 = conPG.cursor()
cur_dump4.execute(dump4)
conPG.commit()

dump5 = r'''COPY (
SELECT 'copy' as test,'G:\S22\'||left(filename,8)||'\'||left(filename,10)||'\ROW-0\'||filename,
'E:\s22\image\'||left(replace(directory,'/','\'),-26)||'\'||filename
FROM survey_image
) TO '%s\%s\copy.bat' ;''' % (path,a)
